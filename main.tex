% !TEX program = xelatex
\documentclass[12pt,twoside]{report} %openright
	%\usepackage[utf8]{inputenc}
 	\usepackage[greek,english]{babel}
    \usepackage{yfonts}
	\usepackage{latexsym}
	\usepackage{amsmath}
	\usepackage{amssymb}
	\usepackage{amsthm}
	\usepackage{stmaryrd}
	\usepackage{pb-diagram}
	\usepackage{amscd}
	\usepackage{mathrsfs}
	\usepackage{enumerate}
	\usepackage{color}
	\usepackage{cite}
	%\usepackage{graphicx}
	\usepackage[all]{xy}
	
	 
	\usepackage{graphicx}
\graphicspath{ {./images/} }
	
	%\usepackage{fixltx2e}
	\usepackage{extarrows}
	\usepackage{datetime}
	\usepackage{mathtools}
    \usepackage{remreset}
    \usepackage{afterpage}
    \usepackage{verbatim}

    \usepackage[svgnames]{xcolor}
	\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    linkcolor=[RGB]{61,82,62},
    filecolor=[RGB]{61,82,62},
    urlcolor=[RGB]{61,82,62},
    citecolor=[RGB]{61,82,62},
    pdftitle={Découpes et isotopies de surfaces topologiques},
    pdflang={fr}
}
\usepackage{bookmark}
    %\usepackage{csquotes}
    \usepackage{etoc}

\usepackage{pdflscape}
	
	\usepackage{fontspec}
	\setmainfont{EB Garamond}
	\usepackage{float}
	\usepackage[12pt]{moresize}
	\usepackage{titlesec}
	\usepackage[twoside,top=1.5in,bottom=1.5in,left=1in,right=1.3in,headheight=16pt,headsep=30pt,footskip=40pt]{geometry}

	\linespread{1.25}
%	\linespread{1.05}


%\NeedsTeXFormat{LaTeX2e}
%\ProvidesPackage{quiver}[2021/01/11 quiver]
\RequirePackage{tikz-cd}
\RequirePackage{amssymb}
\usetikzlibrary{calc}
\usetikzlibrary{decorations.pathmorphing}


% A TikZ style for curved arrows of a fixed height, due to AndréC.
\tikzset{curve/.style={settings={#1},to path={(\tikztostart)
    .. controls ($(\tikztostart)!\pv{pos}!(\tikztotarget)!\pv{height}!270:(\tikztotarget)$)
    and ($(\tikztostart)!1-\pv{pos}!(\tikztotarget)!\pv{height}!270:(\tikztotarget)$)
    .. (\tikztotarget)\tikztonodes}},
    settings/.code={\tikzset{quiver/.cd,#1}
        \def\pv##1{\pgfkeysvalueof{/tikz/quiver/##1}}},
    quiver/.cd,pos/.initial=0.35,height/.initial=0}

% TikZ arrowhead/tail styles.
\tikzset{tail reversed/.code={\pgfsetarrowsstart{tikzcd to}}}
\tikzset{2tail/.code={\pgfsetarrowsstart{Implies[reversed]}}}
\tikzset{2tail reversed/.code={\pgfsetarrowsstart{Implies}}}
% TikZ arrow styles.
\tikzset{no body/.style={/tikz/dash pattern=on 0 off 1mm}}

	% use osf for figures in math mode
	\DeclareSymbolFont{digits}{\encodingdefault}{\rmdefault}{m}{n}
	\SetSymbolFont{digits}{normal}{\encodingdefault}{\rmdefault}{m}{n}
	\DeclareMathSymbol{0}{\mathalpha}{digits}{"30}
	\DeclareMathSymbol{1}{\mathalpha}{digits}{"31}
	\DeclareMathSymbol{2}{\mathalpha}{digits}{"32}
	\DeclareMathSymbol{3}{\mathalpha}{digits}{"33}
	\DeclareMathSymbol{4}{\mathalpha}{digits}{"34}
	\DeclareMathSymbol{5}{\mathalpha}{digits}{"35}
	\DeclareMathSymbol{6}{\mathalpha}{digits}{"36}
	\DeclareMathSymbol{7}{\mathalpha}{digits}{"37}
	\DeclareMathSymbol{8}{\mathalpha}{digits}{"38}
	\DeclareMathSymbol{9}{\mathalpha}{digits}{"39}
	% helvetica for sans in math
	\DeclareMathAlphabet{\mathsf}{\encodingdefault}{phv}{m}{n}
	%\usepackage{showkeys}
	%\usepackage{showframe}
	\usepackage{fancyhdr}
	%\fancyhead[CO]{\rightmark}
	\fancyhead[CO]{}
	%\fancyhead[CE]{\leftmark}
	\fancyhead[CE]{}
	\fancyhead[RE]{}
	\fancyhead[LO]{}
	\fancyhead[RO]{}
	\fancyhead[LE]{}
	\cfoot{\thepage}
	\renewcommand{\headrulewidth}{0.0pt}
	\titleformat\section{\normalfont\Large\centerline{\hss\rule{1.3in}{.01in}\hss}}{\thesection}{1em}{}
    \titleformat{\chapter}[display]{}{\huge\filcenter { \thechapter}}{12pt}{\large \filcenter}

%\newcommand\hmmax{0}
%\newcommand\bmmax{0}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
	% symbols
	\renewcommand{\emptyset}{\varnothing}
	\renewcommand{\phi}{\varphi}
	\newcommand{\isom}{\simeq}
	\newcommand*\isommap{\xlongrightarrow{\sim}}

    \DeclareMathOperator{\Kt}{Kt}
	\DeclareMathOperator{\Ktp}{Ktp}
	\DeclareMathOperator{\KT}{KT}
	\DeclareMathOperator{\GR}{GR}
	\DeclareMathOperator{\id}{id}
	\DeclareMathOperator{\kc}{kc}
	\DeclareMathOperator{\St}{St}
	\DeclareMathOperator{\kcp}{kcp}
	\DeclareMathOperator{\Kcp}{Kcp}
	\DeclareMathOperator{\Aut}{Aut}
	\DeclareMathOperator{\Hom}{Hom}

	\newcommand*\isomlong{%
	  \xlongrightarrow{\raisebox{-0.2 em}{\smash{\ensuremath{\sim}}}}%
	}

		\newcommand{\curvedarrow}[1]{  
		\overset{    
			\raisebox{0.4ex}{      
			\scalebox{1}[-1]{$\scriptstyle\curvearrowright$}    
			}  
		}{#1}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\makeatletter
\newcommand*{\toccontents}{\@starttoc{toc}}
\makeatother

\makeatletter
\@addtoreset{footnote}{section}
\makeatother

%\usepackage{sectsty}
%\partfont{\LARGE}

\begin{document}
\setcounter{tocdepth}{2}
\sloppy

\newgeometry{left=1.5in,right=1.5in, bottom=1.5in}

\thispagestyle{empty}
%\vspace*{\fill}
\noindent\makebox[\linewidth]{\rule{4in}{0.01in}}
\vspace*{\fill}
\begin{center}
    {\Huge Découpes et Isotopies} \\
    \vspace{0.5em}
    {\Huge de Surfaces Topologiques} \\
    \vspace{0.5em}
    {\Large par} \\
    \vspace{0.5em}
    {\huge Yves Ladegaillerie}
\end{center}
\vspace*{\fill}
\noindent\makebox[\linewidth]{\rule{4in}{0.01in}}
%\vspace*{\fill}

\newpage 
\thispagestyle{empty}

\null\vfill
\noindent\begin{minipage}{\textwidth}
This project, involving the transcription of Yves Ladegaillerie's thesis, was carried out under the direction of Mateo Carmona with the collaboration of Fan Xuanrui. The transcription aims to be as faithful as possible to the original. This edition should be regarded as provisional; remarks, comments, and corrections are welcome. More details are available at: \url{https://github.com/gmateocarmona/decoupes-et-isotopies-de-surfaces-topologiques}
\end{minipage}

\newpage

\thispagestyle{empty}
\vspace*{\fill}
\begin{center}
    {\large N$^\circ$ d'enregistrement au C.N.R.S} \\
    \vspace{0.5em}
    {\bf A.0.12452} \\
    \vspace{4em}
    {\large UNIVERSITÉ DES SCIENCES ET TECHNIQUES DU LANGUEDOC} \\
    \vspace{0.5em}
    {\large U.E.R. DE MATHÉMATIQUES} \\
    \vspace{1em}
    1975---1976 \\
	THESE \\
    présentée à l'Université des Sciences et Techniques du Languedoc \\
	pour obtenir le grade de Docteur ès Sciences Mathématiques\\ 
	\vspace{2em}
    {\Large \bf DECOUPES ET ISOTOPIES DE SURFACES TOPOLOGIQUES} \\
    \vspace{2em}
    par \\
    \vspace{2em}
    {\Large Yves LADEGAILLERIE} \\
\end{center}
\vspace*{\fill}
\begin{center}
    Soutenue en Mai 1976 devant la Commission d'examen \\
	\vspace{1em}
    JURY~: MM. A. MICALI \quad\quad Président \\
    \vspace{0.5em}
\begin{tabular}{@{}l@{\hspace{3cm}}r@{}}
N. A'CAMPO & \\ 
C. CONTOU-CARRÈRE & \\ 
A. GROTHENDIECK & Examinateurs \\
M. LEFRANC & \\ 
\end{tabular}
\end{center}
\vspace*{\fill}

\newpage 
\thispagestyle{empty}

\null\vfill
\begin{flushright}
à O.B. et M.L.
\end{flushright}
\vfill\null

\newpage 
\thispagestyle{empty}

\null\vfill
\noindent\begin{minipage}{\textwidth}
L'instigateur de ce travail est Alexandre Grothendieck, bon nombre d'idées qui y sont développées lui sont imputables et l'auteur tient à lui exprimer sa reconnaissance, ainsi qu'à tous ceux qui, à différents titres, lui ont apporté leur aide et notamment~: N. A'Campo, C. Contou-Carrère, P. Damphousse, J.P. Dufour, M. Lefranc, R. Lyndon, A. Micali, C. Mori, J. Saint-Pierre.
\end{minipage}
\vfill\null

\newpage
\thispagestyle{empty}
\mbox{}

%\pagestyle{fancyplain}

\renewcommand{\contentsname}{TABLE DES MATIÈRES}
\tableofcontents\thispagestyle{empty}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\setcounter{page}{0}

\input{./chapitres/0.tex}
\input{./chapitres/1.tex}
\input{./chapitres/2.tex}
\input{./chapitres/3.tex}
\input{./chapitres/4.tex}
\input{./chapitres/5.tex}
\input{./chapitres/6.tex}
\input{./chapitres/7.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage
\mbox{}
\thispagestyle{empty}

\end{document}


%End
